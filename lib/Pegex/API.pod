...
At the most abstract level, Pegex works like this:
    ...
    $result = $parser($grammar, $receiver)->parse($input);
...
receivers job to take action on the data, often building it into some new
structure.

=item L<Pegex::Input>

Pegex abstracts its input stream into an object.

=back

All of these can be subclassed to acheive various results. Normally, you will
write your own Pegex grammar and a Pegex receiver to achieve a task.
...
L<Pegex::AST>. We could have said:

    print Dump pegex($grammar, receiver => 'Pegex::AST')->parse('2+2');
...
The more basic receiver called L<Pegex::Receiver> generates a tree of sequences
...
be precompiled into a module. Pegex make that easy.
...
Now, everytime the grammar module is loaded it will check to see if it needs to
be recompiled, and do it on the fly.
