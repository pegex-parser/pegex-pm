...
The meta statements at the top of a grammar file look like this:

    %pegexKeyword value

Let's look at the top of the pegex.pgx grammar:
...
(Skipping and wrapping are explained in L</Return Values>.)
...
depends on the type of Receiver used.  For example, L<Pegex::Tree> will
return: 
...
into the tree.  (This is different from, say, putting C<undef> into the return.)
...
You can also turn on "wrapping" with the L<Pegex::Tree::Wrap> receiver.
...
Note that this behavior can be "hard set" with the C<+/-> rule modifiers:
    ...
    -<rule>  # Never wrap  (even if using Pegex::Tree::Wrap)
    +<rule>  # Always wrap (even if using Pegex::Tree)

This is simply a check in the C<gotrule> for the receiver.  So, any specific
C<got_*> receiver methods will override even these settings, and choose to pass
the match as-is.  In this case, the C<got_*> sub return value dictates what
